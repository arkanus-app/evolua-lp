...
const copyByLanguage = {
  EN: {
    ...
    badge: "Active class",
    subtitle:
      "Track students, activities, and the teaching rhythm in one workspace for this class.",
    metrics: {
      room: "Room",
      students: "Students",
      average: "Average",
    ...
    students: {
      ...
      subtitle:
        "Manage the class roster, adjust records, and keep each student visible in one operational list.",
      add: "Add student",
      import: "Import list",
      edit: "Edit student",
      average: "Average",
    ...
    activities: {
      ...
      subtitle:
        "Review the full activity roster and open a dedicated form editor when you need to adjust questions.",
  ...
  PT: {
    ...
    badge: "Turma ativa",
    subtitle:
      "Acompanhe alunos, atividades e o ritmo pedagogico em um workspace centralizado para esta turma.",
    metrics: {
      room: "Ambiente",
      students: "Alunos",
      average: "Media",
    ...
    overview: {
      ...
      helper:
        "Use esta area para organizar o ritmo da turma e alinhar os proximos movimentos do grupo.",
    ...
    students: {
      ...
      subtitle:
        "Gerencie a lista da turma, ajuste cadastros e acompanhe cada aluno em uma visao mais operacional.",
      add: "Adicionar aluno",
    ...
    activities: {
      ...
      subtitle:
        "Veja a lista completa de atividades da turma e abra um editor dedicado sempre que precisar ajustar o formulario.",
  ...
  ES: {
    ...
    badge: "Clase activa",
    subtitle:
      "Sigue alumnos, actividades y el ritmo pedagogico en un workspace centralizado para esta clase.",
    metrics: {
      room: "Espacio",
      students: "Alumnos",
      average: "Promedio",
    ...
    overview: {
      ...
      helper:
        "Usa esta area para organizar el ritmo de la clase y alinear los proximos movimientos del grupo.",
    ...
    students: {
      ...
      subtitle:
        "Gestiona la lista de la clase, ajusta registros y acompana a cada alumno en una vista mas operativa.",
      add: "Agregar alumno",
      import: "Importar lista",
      edit: "Editar alumno",
      average: "Promedio",
    ...
    activities: {
      ...
      subtitle:
        "Consulta la lista completa de actividades de la clase y abre un editor dedicado cuando necesites ajustar el formulario.",
...
export default function TurmaDetalhePage() {
  ...
  const createActivityHref = `/turmas/${params.id}/atividades/nova`;

  useEffect(() => {
    const loadClassWorkspace = async () => {
      setIsLoading(true);
      setError(null);

      try {
        const [summaryResponse, studentsResponse, activitiesResponse] = await Promise.all([
          api.getTurmaResumo({
        ...
        if (summaryResponse.status !== 200) {
          setError(getApiErrorMessage(summaryResponse));
          return;
        }

        if (studentsResponse.status !== 200) {
          setError(getApiErrorMessage(studentsResponse));
          return;
        }

        if (activitiesResponse.status !== 200) {
          setError(getApiErrorMessage(activitiesResponse));
          return;
        }

        setCurrentClass(summaryResponse.body.class);
        setPulse({
          attendanceRate: summaryResponse.body.pulse.attendanceRate ?? 0,
          deliveryRate: summaryResponse.body.pulse.deliveryRate ?? 0,
          engagementRate: summaryResponse.body.pulse.engagementRate ?? 0,
        });
        setStudents(studentsResponse.body.items);
        setActivities(activitiesResponse.body.items);
      } catch (requestError) {
        setError(
          requestError instanceof Error
            ? requestError.message
            : "Nao foi possivel carregar a turma.",
        );
      } finally {
        setIsLoading(false);
      }
          ...
          <div className="grid gap-6 xl:grid-cols-[1.4fr_0.9fr]">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {[
                ...
                {
                  label: copy.metrics.students,
                  value: String(currentClass.students),
                },
                {
                  label: copy.metrics.average,
          ...
          {activeTab === "overview" && (
            ...
            >
              <div className="rounded-[2rem] border border-slate-200 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-900 p-6 text-white">
                <div className="text-xs font-black uppercase tracking-[0.24em] text-brand-yellow">
                  {copy.overview.title}
                </div>
                <h2 className="mt-3 text-2xl font-extrabold tracking-tight">
                  {currentClass.nextTopic}
                </h2>
                <p className="mt-4 max-w-xl text-sm font-semibold leading-relaxed text-slate-300">
                  {copy.overview.helper}
                </p>

                <div className="mt-6 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-[1.6rem] border border-white/10 bg-white/5 p-4">
                    <div className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">
                      {copy.overview.schedule}
                    </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs font-semibold text-slate-500">
                              {statusLabelByStudent[student.status]}
                            </div>
                          </div>
                        </div>
                ...
                <div className="rounded-[2rem] border border-slate-200 bg-white p-5">
                  ...
                  <div className="mt-4 space-y-3">
                    {[
                      ...
                      {
                        label: copy.overview.deliveries,
                        value: pulse.deliveryRate,
                      },
                      {
                        label: copy.overview.engagement,
                        value: pulse.engagementRate,
                      },
                    ].map((item) => (
                      <div key={item.label}>
                        ...
                        <div className="h-3 rounded-full bg-slate-100">
                          <div
                            className="h-3 rounded-full bg-gradient-to-r from-brand-yellow to-brand-yellowDark"
            ...
            >
              ...
              <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
                <div className="hidden grid-cols-[2.2fr_1fr_0.8fr_0.9fr_0.9fr] gap-4 border-b border-slate-200 bg-slate-50 px-5 py-4 lg:grid">
                  <div className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">
                    {copy.students.columns.student}
                  </div>
                  <div className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">
                    {copy.students.columns.status}
                  </div>
                  <div className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">
                    {copy.students.columns.average}
                  </div>
                  <div className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">
                    {copy.students.columns.engagement}
                  </div>
                  <div className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">
                    {copy.students.columns.actions}
                  </div>
                </div>
                ...
                {students.map((student, index) => (
                  <div
                    key={student.id}
                    className={`border-slate-200 px-5 py-4 ${index !== students.length - 1 ? "border-b" : ""}`}
                  >
            ...
            >
              ...
              <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
                <div className="hidden grid-cols-[2fr_0.9fr_1fr_0.8fr_0.9fr] gap-4 border-b border-slate-200 bg-slate-50 px-5 py-4 lg:grid">
                  <div className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">
                    {copy.activities.columns.activity}
                  </div>
                  <div className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">
                    {copy.activities.columns.status}
                  </div>
                  <div className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">
                    {copy.activities.columns.deadline}
                  </div>
                  <div className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">
                    {copy.activities.columns.questions}
                  </div>
                  <div className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">
                    {copy.activities.columns.actions}
                  </div>
                </div>

                {activities.map((activity, index) => {
                  const editHref = `/turmas/${params.id}/atividades/${activity.id}`;

                  return (
                    <div
                      key={activity.id}
                      className={`border-slate-200 px-5 py-4 ${index !== activities.length - 1 ? "border-b" : ""}`}
                    >
                      <div className="hidden items-center gap-4 lg:grid lg:grid-cols-[2fr_0.9fr_1fr_0.8fr_0.9fr]">
                        ...
                        <div className="text-sm font-bold text-slate-700">
                          {statusLabelByActivity[activity.status]}
                        </div>
                        <div className="text-sm font-semibold text-slate-500">
                          {activity.dueDate}
                        </div>
