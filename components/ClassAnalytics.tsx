...
import type { ClassAnalytics as ClassAnalyticsDto } from "../lib/api/contract";

export const ClassAnalytics: React.FC = () => {
  const { t } = useLanguage();
  const [activeSkill, setActiveSkill] = useState<string | null>(null);
  const [isMounted, setIsMounted] = useState(false);
  const [analytics, setAnalytics] = useState<ClassAnalyticsDto | null>(null);
  ...
  const data = (analytics?.radar ?? []).map((item) => ({
    subject: t(`analytics.topic.${item.fullKey}`),
              ...
              <div className="flex justify-between items-center mb-2 px-2 shrink-0">
                ...
                <div className="flex gap-3 text-xs font-bold">
                  <div className="flex items-center gap-1">
                    ...
                    {t("analytics.chart.class")}
                  </div>
                  <div className="flex items-center gap-1">
                    <div className="w-3 h-3 rounded-full bg-purple-500"></div>{" "}
                    {t("analytics.chart.student")}
                  </div>
                </div>
              </div>
                ...
                {isMounted ? (
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <Radar
                        name={t("analytics.chart.student")}
                        dataKey="A"
                        stroke="#a855f7"
                        strokeWidth={3}
                        fill="#a855f7"
                    ...
                    <div className="flex gap-4 text-xs">
                      <div className="flex-1">
                        ...
                        <span className="text-lg font-black text-purple-600">
                          {Math.round((currentSkill.A / 150) * 100)}%
                        </span>
                      </div>
                      <div className="flex-1">
                        <span className="block text-slate-400 font-bold">
                          {t("analytics.classAvg")}
                        </span>
                        <span className="text-lg font-black text-brand-yellow">
                          {Math.round((currentSkill.B / 150) * 100)}%
                        </span>
                      </div>
                    </div>
