...
import type { ClassAnalytics } from "../lib/api/contract";
...
export const DashboardPreview: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadAnalytics = async () => {
      const response = await api.getTurmaAnalytics({
        params: { turmaId: "1" },
        query: { period: "30d" },
      });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="order-2 lg:order-1">
            ...
            <p className="text-lg text-slate-600 mb-8 font-medium">{t("dash.subtitle")}</p>

            <div className="space-y-6 mb-10">
              {[t("dash.list.1"), t("dash.list.2"), t("dash.list.3"), t("dash.list.4")].map(
                (item, i) => (
                  <motion.div
                    key={i}
                    whileHover={{ x: 10 }}
              ...
              <div className="p-8">
                <div className="grid grid-cols-3 gap-4 mb-8">
                  <div className="bg-yellow-50 p-5 rounded-2xl border border-yellow-100">
                    <div className="text-xs text-yellow-700 font-bold uppercase mb-1">
                      {t("dash.vis.avg")}
                    </div>
                    <div className="text-3xl font-black text-slate-900">
                      {dashboard ? `${dashboard.average.toFixed(1)}%` : "--"}
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                    ...
                    <div className="text-3xl font-black text-slate-900 text-slate-300">
                      {dashboard?.pending ?? "--"}
                    </div>
                  </div>
                </div>
