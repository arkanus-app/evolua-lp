...
export const ParentPortal: React.FC = () => {
  ...
  useEffect(() => {
    const loadNotifications = async () => {
      const response = await api.listResponsavelNotificacoes({
        params: { responsavelId: "guardian-1" },
        query: { studentId: "1-student-1", limit: 10 },
      });

      if (response.status === 200) {
        setNotifications(response.body.items);
        setExpandedId(null);
      }
    };
  ...
  const triggerNotification = async () => {
    const templates = [
      {
        type: "message",
        title: t("parent.dynamic.message.title"),
        subtitle: "Mrs. Robinson",
        value: "1",
      ...
      },
      {
        ...
        subtitle: t("parent.dynamic.grade.subtitle"),
        value: "A-",
        color: "bg-brand-yellow text-slate-900",
      },
      {
        type: "award",
        title: t("parent.dynamic.award.title"),
        subtitle: t("parent.dynamic.award.subtitle"),
        value: "Lvl 5",
        color: "bg-green-100 text-green-700",
      },
      {
        type: "attendance",
        title: t("parent.dynamic.attendance.title"),
        subtitle: t("parent.dynamic.attendance.subtitle"),
        value: "OK",
        color: "bg-slate-100 text-slate-700",
      },
    ];

    const randomTemplate = templates[templateIndex % templates.length];
    setTemplateIndex((current) => current + 1);
    const response = await api.createNotification({
      body: {
        studentId: "1-student-1",
        type: randomTemplate.type as "message" | "grade" | "award" | "attendance",
        title: randomTemplate.title,
        message: getDesc(randomTemplate.type),
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <div className="order-1">
            ...
            <ul className="space-y-4 mb-10">
              {[t("parent.list.1"), t("parent.list.2"), t("parent.list.3")].map((feat, i) => (
                <motion.li
                  key={i}
                ...
                <div className="flex-1 space-y-3 pb-20 relative overflow-y-auto no-scrollbar">
                  <AnimatePresence initial={false} mode="popLayout">
                    {notifications.map((notif) => (
                      ...
                      >
                        <div
                          className={`absolute top-0 left-0 w-1 h-full ${notif.type === "grade" ? "bg-brand-yellow" : "bg-blue-500"}`}
                        ></div>
                        ...
                        <div className="p-4">
                          ...
                          <div className="pl-9 flex justify-between items-center">
                            <div>
                              ...
                              <div
                                className={`inline-block text-xs font-extrabold px-2 py-0.5 rounded-md ${notif.color}`}
                              >
                                {notif.value}
                              </div>
                            </div>
