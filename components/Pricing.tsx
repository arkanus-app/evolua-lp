...
import type { PricingPlan } from "../lib/api/contract";
import { Button } from "./Button";

export const Pricing: React.FC = () => {
  const { t, language } = useLanguage();
  const [isAnnual, setIsAnnual] = useState(false);
  const [tiers, setTiers] = useState<PricingPlan[]>([]);

  useEffect(() => {
    const loadPricing = async () => {
      const response = await api.getPublicPricing({
        query: {
          locale: language,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h3 className="text-4xl md:text-5xl font-extrabold text-slate-900 tracking-tight mb-6">
            {t("pricing.title")}{" "}
            <span className="text-brand-yellowDark">{t("pricing.title.span")}</span>
          </h3>
          <p className="text-lg text-slate-600 font-medium mb-10">{t("pricing.subtitle")}</p>
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <span
              className={`text-sm font-bold ${isAnnual ? "text-slate-900" : "text-slate-500"} flex items-center gap-2`}
            >
              {t("pricing.yearly")}
                ...
                <div className="flex items-baseline gap-1 mb-4">
                  ...
                  {tier.interval !== "custom" && (
                    <span className="text-slate-500 font-medium">
                      /{tier.interval === "annual" ? t("pricing.yearly") : t("pricing.monthly")}
                    </span>
                  )}
                </div>
                ...
                <ul className="space-y-4">
                  {tier.features.map((feat, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm font-bold text-slate-700">
                      <div
                        className={`p-0.5 rounded-full ${tier.highlight ? "bg-brand-yellow/20 text-brand-yellowDark" : "bg-slate-100 text-slate-500"}`}
                      >
