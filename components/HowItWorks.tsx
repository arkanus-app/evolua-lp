...
import { Check, CloudUpload, FileText, Smartphone, X, Zap } from "lucide-react";
...
export const HowItWorks: React.FC = () => {
  ...
  const defaultCorrectAnswers = [1, 0, 2, 0, 1, 2]; // B, A, C, A, B, C
  const defaultAnswers = [1, 0, 3, 0, 2, 2];
  const defaultStudentId = "24018";

  const [userAnswers, setUserAnswers] = useState(defaultAnswers);
  const [correctAnswers, setCorrectAnswers] = useState(defaultCorrectAnswers);
  const [studentId, setStudentId] = useState(defaultStudentId);
  const [currentSubject, setCurrentSubject] = useState(subjects[0] ?? "Matematica");
      ...
      setTimeout(() => {
        ...
        void api
          ...
          .then((response) => {
            ...
            setUserAnswers(response.body.answers);
            setCorrectAnswers(response.body.correctAnswers);
            setStudentId(response.body.studentId);
            setCurrentSubject(response.body.subject);
          });
      }, 4000);
    },
    [isUploading, showResults, currentSubject],
  );
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-12 md:mb-20">
          <h2 className="text-sm font-black text-brand-yellowDark tracking-widest uppercase mb-3">
            {t("hiw.eyebrow")}
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            >
              ...
              <div className="absolute inset-0 bg-slate-800 flex flex-col overflow-hidden rounded-[2.5rem] md:rounded-[2.8rem]">
                                ...
                                {["A", "B", "C", "D"].map((opt, col) => {
                                  ...
                                  return (
                                    <div key={col} className="relative">
                                      <div
                                        className={`w-6 h-6 md:w-8 md:h-8 rounded-full border-2 transition-colors duration-300 ${isSelected ? "bg-slate-800 border-slate-800" : "border-slate-200"}`}
                                      />
                ...
                <div className="h-20 md:h-24 bg-black/90 backdrop-blur-md flex items-center justify-center gap-6 md:gap-8 relative z-30">
                  ...
                  >
                    <div
                      className={`w-12 h-12 md:w-14 md:h-14 rounded-full transition-colors ${isUploading ? "bg-slate-500" : "bg-white group-active:bg-slate-200"}`}
                    />
                  </button>
                  <button
                  ...
                  >
                    <Zap
                      className={`w-5 h-5 transition-colors duration-300 ${isFlashlightOn ? "text-yellow-400 fill-yellow-400" : "text-slate-500"}`}
                    />
                  </button>
                </div>
              </div>
          ...
          <div className="order-1 lg:order-2 space-y-8 md:space-y-10">
            {[
              ...
              >
                <div
                  className={`flex-shrink-0 w-14 h-14 md:w-16 md:h-16 ${step.color} rounded-[1.2rem] border-b-4 border-black/10 flex items-center justify-center shadow-lg transform transition-all duration-300 group-hover:scale-110 group-hover:rotate-6 group-hover:-translate-y-1`}
                >
                  {step.icon}
                </div>
                <div>
