...
import { Layers, ChevronRight, BookOpen, Microscope, Calculator, Globe2, Settings2, GripVertical, ArrowRight, Wand2, Check } from 'lucide-react';
...
export const CurriculumBreakdown: React.FC = () => {
  ...
  const data: Area[] = [
    {
      id: 'humanities',
      labelKey: 'curr.area.humanities',
      color: 'bg-orange-500',
      disciplines: [
        {
          id: 'history',
          labelKey: 'curr.disc.history',
          icon: BookOpen,
          topics: [
            { id: 'indrev', labelKey: 'curr.topic.indrev', efficiency: 92 },
            { id: 'stoneage', labelKey: 'curr.topic.stoneage', efficiency: 76 },
            { id: 'coldwar', labelKey: 'curr.topic.coldwar', efficiency: 88 },
          ]
        },
        {
          id: 'geography',
          labelKey: 'curr.disc.geography',
          icon: Globe2,
          topics: [
            { id: 'mapreading', labelKey: 'curr.topic.mapreading', efficiency: 65 },
            { id: 'urban', labelKey: 'curr.topic.urban', efficiency: 82 },
          ]
        }
      ]
    },
    {
      id: 'stem',
      labelKey: 'curr.area.stem',
      color: 'bg-blue-500',
      disciplines: [
        {
          id: 'math',
          labelKey: 'curr.disc.math',
          icon: Calculator,
          topics: [
            { id: 'algebra', labelKey: 'curr.topic.algebra', efficiency: 74 },
            { id: 'calc', labelKey: 'curr.topic.calc', efficiency: 68 },
          ]
        },
        {
          id: 'physics',
          labelKey: 'curr.disc.physics',
          icon: Microscope,
          topics: [
            { id: 'mech', labelKey: 'curr.topic.mech', efficiency: 85 },
            { id: 'thermo', labelKey: 'curr.topic.thermo', efficiency: 91 },
          ]
        }
      ]
    }
  ...
  const selectedDiscipline = selectedArea.disciplines.find(d => d.id === selectedDisciplineId) || selectedArea.disciplines[0];
  ...
  }, [selectedAreaId, selectedArea.disciplines, selectedDisciplineId]);
       ...
       <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch min-h-[400px]">
            ...
            <div className="md:col-span-3 flex flex-col gap-4">
               <div className="text-xs font-bold text-slate-400 uppercase tracking-widest px-2">{t('curr.col1')}</div>
               <LayoutGroup>
                 {data.map((area) => (
                   <motion.button
                     layout
                   ...
                   >
                     ...
                     <span className={`text-lg font-black ${selectedAreaId === area.id ? 'text-slate-900' : 'text-slate-500'}`}>
                       {t(area.labelKey)}
                     </span>
                     {selectedAreaId === area.id && (
                       <motion.div 
            ...
            <div className="md:col-span-4 flex flex-col gap-4">
              <div className="text-xs font-bold text-slate-400 uppercase tracking-widest px-2">{t('curr.col2')}</div>
              <AnimatePresence mode="wait">
                ...
                >
                  {selectedArea.disciplines.map((disc) => (
                    ...
                    >
                       <div className={`p-2 rounded-lg ${selectedDisciplineId === disc.id ? 'bg-slate-100' : 'bg-white'}`}>
                          <disc.icon className="w-5 h-5" />
                       </div>
                       <span className="font-bold text-sm flex-1 text-left">{t(disc.labelKey)}</span>
                       {selectedDisciplineId === disc.id && <ChevronRight className="w-4 h-4 text-slate-400" />}
                    </motion.button>
                  ))}
                </motion.div>
              </AnimatePresence>
            ...
            <div className="md:col-span-5 flex flex-col gap-4">
              <div className="text-xs font-bold text-slate-400 uppercase tracking-widest px-2">{t('curr.col3')}</div>
              <div className="bg-white rounded-[2rem] border border-slate-200 shadow-xl p-6 h-full flex flex-col relative overflow-hidden">
                 ...
                 <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-100 relative z-10">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="font-bold text-slate-700 text-sm">{t(selectedDiscipline.labelKey)}</span>
                    </div>
                    <div className="text-xs font-bold text-slate-400">{t('curr.metric.efficiency')}</div>
                 </div>
                    ...
                    <AnimatePresence mode="popLayout">
                       {selectedDiscipline.topics.map((topic, i) => (
                          <motion.div 
                             ...
                             animate={isEditMode 
                                ? { opacity: 1, y: 0, x: i % 2 === 0 ? 1 : -1, rotate: i % 2 === 0 ? 0.5 : -0.5 } 
                                : { opacity: 1, y: 0, x: 0, rotate: 0 }
                             }
                             transition={isEditMode 
                                ? { 
                                    x: { repeat: Infinity, repeatType: "reverse", duration: 0.2 }, 
                                    rotate: { repeat: Infinity, repeatType: "reverse", duration: 0.25 }
                                  } 
                          ...
                          >
                             ...
                             <div className={isEditMode ? "pl-6 pointer-events-none" : ""}>
                                <div className="flex justify-between text-sm font-bold text-slate-800 mb-2">
                                    ...
                                    <span className={topic.efficiency < 70 ? 'text-red-500' : 'text-green-600'}>
                                    {topic.efficiency}%
                                    </span>
                                </div>
                                <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
                                    ...
                                    className={`h-full rounded-full ${
                                        topic.efficiency > 85 ? 'bg-green-500' : 
                                        topic.efficiency > 70 ? 'bg-brand-yellow' : 
                                        'bg-red-400'
                                    }`}
                                    />
                                </div>
                 ...
                 <div className="mt-6 pt-4 border-t border-slate-100 text-center relative z-10">
                    ...
                    <div className="text-2xl font-black text-slate-900">
                      {Math.round(selectedDiscipline.topics.reduce((acc, curr) => acc + curr.efficiency, 0) / selectedDiscipline.topics.length)}%
                    </div>
                 </div>
              </div>
            </div>
